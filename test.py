    ...
    with torch.no_grad():
        for run in range(_config['n_runs']):
            ...
            for sample_batched in tqdm.tqdm(testloader):
                ...
                plt.figure()
                plt.subplot(2, 2, 1)
                plt.imshow(np.array(query_pred.argmax(
                    dim=1)[0].cpu()), cmap=cm.tab10_r)
                plt.subplot(2, 2, 2)
                plt.imshow(np.array(query_labels[0].cpu()), cmap=cm.tab10_r)
                plt.subplot(2, 2, 3)
                plt.imshow(
                    np.array(np.squeeze(query_images[0].cpu())).transpose(1, 2, 0))
                plt.axis('off')
                plt.savefig("./outputs/%d.png" % counter)
                plt.show()
